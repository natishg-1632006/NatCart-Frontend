...
import DellImg from "/Image/Dell.jpg"

const LaptopsCards = (props) => {
    const {LaptopData}=props;
    const {name,brand,price,offerPrice}=LaptopData;
  return (
    <>
      <div className='m-5'>
        <div className='h-80 w-60 bg-white shadow-md rounded-md duration-1000 hover:shadow-lg cursor-pointer'>
            <div className='flex justify-center'>
                <img src={laptoImg} alt="Laptop image" className='h-40'/>
            </div>
            <div className='p-3 flex flex-col gap-2'>
            <div className='flex flex-col gap-2'>
                <p className='font-semibold text-green-700 line-clamp-1'>{name}</p>
                <p>Brand: {brand}</p>
